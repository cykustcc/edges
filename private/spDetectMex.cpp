...
void sticky( int *S, int h, int w, float *I, float *E, double *prm ) {
  // get additional parameters
  const int maxIter=int(prm[0]); int nThreads=int(prm[1]);
  const float sigs=float(prm[2]), sigx=float(prm[3]);
  const float sige=float(prm[4]), sigc=float(prm[5]);

  // initialize mus and ns
  int m=0; for( int x=0; x<w*h; x++ ) m=S[x]>m ? S[x] : m; m++;
  float *ns=new float[m](), *mus=new float[m*5]();
  for( int x=0; x<w; x++ ) for( int y=0; y<h; y++ ) {
    int i=S[x*h+y]; ns[i]++; mus[i*5]+=x; mus[i*5+1]+=y;
    for(int z=0; z<3; z++) mus[i*5+z+2]+=I[z*h*w+x*h+y];
  }
  for(int i=0; i<m; i++) for(int z=0; z<5; z++) mus[i*5+z]/=ns[i];
  ...
  while( changed && iter<maxIter ) {
    changed=0;
    #ifdef USEOMP
    nThreads = min(nThreads,int(omp_get_max_threads()));
    #pragma omp parallel for num_threads(nThreads)
    #endif
    for( int xi=0; xi<int(w); xi++ ) for( int y=0; y<h; y++ ) {
      // get local region, if all uniform continue
      int x=xi, x0, x1, y0, y1, i, j, s=S[x*h+y], t, sBest, T[4];
...
void relabel( int *S, int h, int w ) {
  int *T=new int[h*w], *map=new int[h*w/2]();
  int x, y, z, t, t1, t2, m=1;
  ...
  int m1=0; for( t=1; t<m; t++ ) map[t] = map[t] ? map[map[t]] : m1++;
  for( x=0; x<w*h; x++ ) S[x]=map[T[x]];
  delete [] T; delete [] map;
}

// add or remove superpixel boundaries (and make superpixels 1 or 0-indexed)
void boundaries( int *S, int h, int w, float *E, bool add, int nThreads ) {
  // helper for getting local neighborhood
  #define NEIGHBORS8(S) int *L=S+x*h+y; int x0, y0, x1, y1; \
    x0 = (x==0) ? 0 : -1; x1 = (x==w-1) ? 0 : 1; x0*=h; x1*=h; \
    y0 = (y==0) ? 0 : -1; y1 = (y==h-1) ? 0 : 1; \
    int N[8]={L[x0],L[x1],L[y0],L[y1],L[x0+y0],L[x0+y1],L[x1+y0],L[x1+y1]};
  if( add ) {
    // make S 1-indexed
    ...
    #endif
    for( int xi=0; xi<int(w); xi++ ) for( int y=0; y<h; y++ ) {
      int x=xi, a=x*h+y, b=(x+1)*h+y, c=x*h+(y+1), s=S[a]; if(s==0) continue;
    ...
    #endif
    for( int xi=0; xi<int(w); xi++ ) for( int y=0; y<h; y++ ) {
      int x=xi; NEIGHBORS8(S); int i, s=L[0]; if(s==0) continue;
    ...
    // make S 0-indexed
    int *T=new int[h*w]; memcpy(T,S,h*w*sizeof(int));
    #ifdef USEOMP
    #pragma omp parallel for num_threads(nThreads)
    #endif
    for( int xi=0; xi<int(w); xi++ ) for( int y=0; y<h; y++ ) {
      int x=xi; NEIGHBORS8(T); int i, *s=&S[x*h+y]; if(*s!=0) continue;
    ...
    for( int x=0; x<w*h; x++ ) if(S[x]) S[x]--; delete [] T;
  }
}

// merge superpixels that are separated by a weak boundary
void merge( int *S, int h, int w, float *E, float thr ) {
  // compute m and min for each region
  int x; int m=0; for( x=0; x<w*h; x++ ) m=S[x]>m ? S[x] : m; m++;
  ...
  for( x=0; x<w; x++ ) for( int y=0; y<h; y++ ) if( S[x*h+y]==0 ) {
    int i, j, s, s1, s2, k=0, U[8]; NEIGHBORS8(S);
  ...
  int m1=1; for( int s=1; s<m; s++ ) map[s] = map[s] ? map[map[s]] : m1++;
  for( x=0; x<w*h; x++ ) if(S[x]) S[x]=map[S[x]];
  for( x=0; x<w; x++ ) for( int y=0; y<h; y++ ) if( S[x*h+y]==0 ) {
    int i, s=0; NEIGHBORS8(S); for(i=0; i<8; i++) if(N[i]) { s=N[i]; break; }
...
void visualize( float *V, int *S, int h, int w, float *I, bool hasBnds ) {
  int i, z, n=h*w;
  int m=0; for( int x=0; x<w*h; x++ ) m=S[x]>m ? S[x] : m; m++;
  float *clrs=new float[m]; int *cnts=new int[n];
...
void affinities( float *A, int *S, int h, int w, float *E,
  int8 *segs, int *dims, int nThreads )
{
  const int g=dims[0], h1=dims[2], w1=dims[3], nTreesEval=dims[4];
  const int stride=int( float(h)/float(h1) + .5f );
  int m=0; for( int x=0; x<w*h; x++ ) m=S[x]>m ? S[x] : m; m++;
  float *wts=new float[w*h], *Sn=new float[m*m](), *Sd=new float[m*m]();
  for(int i=0; i<w*h; i++) wts[i]=1/(1+exp((E[i]-.05f)*50.f));
  #ifdef USEOMP
  nThreads = min(nThreads,int(omp_get_max_threads()));
  #pragma omp parallel for num_threads(nThreads)
  #endif
  for( int x=0; x<int(w); x+=stride ) {
    ...
    int i, j, s, m1, s1, t, lbl, ind, nTreesConst;
    int *lookup, *lbls1; float *wts1;
    lookup=new int[m]; lbls1=new int[g*g]; wts1=new float[m*25];
    for( y=0; y<int(h); y+=stride ) {
      ...
      for( t=0; t<nTreesEval; t++ ) {
        ...
        const int8 *seg=segs+ind*g*g; s1=0;
  ...
  for( int s=1; s<m; s++ ) for( int t=1; t<m; t++ ) if( Sd[s*m+t] ) {
...
void edges( float *E, int *S, int h, int w, float *A ) {
  int x, y, x0, x1, xi, y0, y1, yi, i, j, s, n, ss[9];
  int m=0; for( x=0; x<w*h; x++ ) m=S[x]>m ? S[x] : m;
...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] ) {
  ...
  if(f) { mexErrMsgTxt("Failed to get action.");
    ...
    pl[0] = mxCreateNumericMatrix(h,w,mxINT32_CLASS,mxREAL);
    int* T = (int*) mxGetData(pl[0]); memcpy(T,S,h*w*sizeof(int));
    ...
    int nThreads = (int) mxGetScalar(pr[3]);
    pl[0] = mxCreateNumericMatrix(h,w,mxINT32_CLASS,mxREAL);
    int* T = (int*) mxGetData(pl[0]); memcpy(T,S,h*w*sizeof(int));
    ...
    float *E  = (float*) mxGetData(pr[1]);
    int8 *segs  = (int8*) mxGetData(pr[2]);
    int nThreads = (int) mxGetScalar(pr[3]);
    if( mxGetNumberOfDimensions(pr[2])!=5 ) mexErrMsgTxt("invalid input");
    int *dims = (int*) mxGetDimensions(pr[2]);
    int m=0; for( int x=0; x<w*h; x++ ) m=S[x]>m ? S[x] : m;
